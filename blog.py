...
class LoginForm(Form):
    ...
    password = PasswordField("Parolanızı giriniz:",validators=[])
...
def detail(id):
    ...
    if result >0:
        ...
        return render_template("arrticle.html",article = article)
    else:
        return render_template("arrticle.html")
...
def search():
    if request.method == "GET":
        ...
        if result == 0:
            flash("Böyle bir söyleşi dinlemedim!","warning")
            return redirect(url_for("articles"))
        else:
            articles = cursor.fetchall()
...
class ArticleForm(Form):
    username = StringField("Yazarın Adı:",validators=[validators.length(min=4,max=16)])
    title = StringField("Makale Başlığı",validators=[validators.length(min=5,max=30)])
    content = TextAreaField("Makale İçeriği",validators=[validators.length(min=30,max=2000)])
